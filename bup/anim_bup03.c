...
int main (){ 
	...
	if (output_zip[0]){ 
		...
		DeleteFile (output_zip); 
		if (output_png[0]) sprintf (extra_string, "7za a -tzip -mx9 %s %s*.png", output_zip, output_png); 
		else if (output_bmp[0]) sprintf (extra_string, "7za a -tzip -mx9 %s %s*.png", output_zip, output_png); 
		else { 
			extra_string[0]= 0; 
		...
		if (CreateProcess ("C:\\7Z\\7za.exe", extra_string, 0, 0, 0, CREATE_NO_WINDOW, 0, 0, &s_i, &p_i)){ 
			do { 
