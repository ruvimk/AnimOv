...
void MainInit (){ 
	...
	if (InputLLC){ 
		...
		if (!strstr (llc_file, ".lcb")){ 
			strcat (llc_file, ".lcb"); 
		} 
		llcbuf2= (long *)(file_get_contents (llc_file)); 
		GlobalFree (InputLLC); 
	} 
	// if (debug_mode) printf ("place marker %d\r\n", 2); 
...
int main (){ 
	...
	if (output_zip[0]){ 
		...
		/*if (output_png[0]) */sprintf (extra_string, "7za a -tzip -mx9 ../%s *.png", output_zip, id); 
