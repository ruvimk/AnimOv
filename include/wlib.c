...
int iHexNibble (int hex){ 
	if (hex >= 48 && hex < 58) return hex - 48; 
	return (hex | 32) - 87; 
} 
void EnsureNoQuotes (char * txt){ 
	char * p; 
	if (txt[0] == 34){ 
		...
		while (*p){ 
			...
			if (*p == 92){ 
				if (p[1] >= 48 && p[1] < 58){ 
					...
					*p= iHexNibble (p[2]) * 16 + iHexNibble (p[3]); 
					memmove (p + 1, p + 4, strlen (p + 3)); 
					p++; 
				} else if (p[1] == 92){ 
					memmove (p + 1, p + 2, strlen (p + 1)); 
					p++; 
...
void raycast_debug (int a, int b, int c){ 
	char string1[512]; 
	char string2[512]; 
	unsigned long bW; 
	HANDLE hFile= (void *)(-2); 
	do { 
		// if (hFile != -2) Sleep (10); 
		hFile= CreateFile ("raycast_debug.txt", GENERIC_WRITE | GENERIC_READ, 0, 0, 128, OPEN_ALWAYS, 0); 
		if (GetLastError () != ERROR_SHARING_VIOLATION) return; 
	} while (!hFile || (int)(hFile) == -1); 
	if (a || b || c){ 
		sprintf (string1, "Raycast, x=%d, y=%d, result=%d \r\n", a, b, c); 
	} else { 
		strcpy (string1, "\r\n-------------------------- break ----------------------------- \r\n"); 
	} 
	WriteFile (hFile, string1, strlen (string1), &bW, 0); 
	// sprintf (string2, "data: debug:%s\r\n", string1); 
	// cprint (string2); 
	CloseHandle (hFile); 
} 

int Ellipsoid_RayCast (int x, int y, double * matRinv, POINT3D * primeP, long * last_depth){ 
	DEPTH_PIXEL * px; 
	double matE[3]; 
	double matD[3]; 
	double matE1[3]; 
	...
	double t1; 
	double t2; 
	// long last_depth= (long)(sqrt (pow (atX, 2) + pow (atY, 2) + pow (atZ, 2)) * DepthScalar); 
	long outline_rel_depth= DepthScalar / 24; 
	double zm= pow (0.9, cam_zoom); 
	if (x < 0 || x >= main_w) return -2; 
	if (y < 0 || y >= main_h) return -2; 
	...
	if ((int)(px) >= (int)(BitmapData) && (int)(BitmapData) + (main_w * main_h * 8) > (int)(px)){ 
		...
		if (RayEllipsoid (axisX, axisY, axisZ, 
			...
			if (last_depth) *last_depth= (long)(t * DepthScalar); 
		} else if (stroke_outlines && objLine && last_depth){ 
			if (px->depth == PX_UNDEFINED || ((px->depth) > (*last_depth + outline_rel_depth))){ 
				px->r= objOutline[0]; 
				px->g= objOutline[1]; 
				px->b= objOutline[2]; 
				px->depth= (*last_depth + outline_rel_depth); 
			} 
			return 0; 
		} else if (UseDebugBox){ 
...
void RenderEllipsoid (){ 
	...
	int tmp03; 
	int unknown_count; 
	long lastd= (long)(sqrt (pow (atX, 2) + pow (atY, 2) + pow (atZ, 2)) * DepthScalar); 
	...
	*/
	...
	centerPT.y= primeP.z; 
	centerPT.z= primeP.y; 
	OpticTangentTransform (&centerPT, 1, main_w, main_h, cam_rh, cam_rv, cam_zoom); 
	centerX= centerPT.x; 
	centerY= centerPT.y; 
	// if (centerX < 0 || centerX >= main_w) return; 
	// if (centerY < 0 || centerY >= main_h) return; 
	run_loop_1= 1; 
	for (i= centerY; run_loop_1; i++){ 
		run_loop_2= 1; 
		minD= maxD= centerX; 
		tmp02= 0; 
		unknown_count= 0; 
		for (j= centerX; run_loop_2; j++){ 
			tmp01= Ellipsoid_RayCast (j, i, matRinv, &primeP, &lastd); 
			if (tmp01 > 0){ 
				maxD= j; 
				tmp02++; 
			} else if (!tmp01){ 
				...
				unknown_count++; 
				#ifdef MAX_UNKNOWN_X 
					if (unknown_count > MAX_UNKNOWN_X){ 
						run_loop_2= 0; 
						continue; 
					} 
				#else 
		...
		// tmp02 += centerX - j; 
		// if (maxD - minD < 3) run_loop_1= 0; 
		if (!tmp02) run_loop_1= 0; 
		centerX= (maxD - minD) / 2 + minD; 
	} 
	centerX= centerPT.x; 
	run_loop_1= 1; 
	for (i= centerY - 1; run_loop_1; i--){ 
		run_loop_2= 1; 
		minD= maxD= centerX; 
		tmp02= 0; 
		unknown_count= 0; 
...
void StrokeCurve (){ 
	...
	for (i= 0; (i + 1) * 3 < len; i += 4){ 
		...
		b_z= 3 * ((c->z) - (b->z)) - c_z; 
		a_z= (d->z) - (a->z) - c_z - b_z; 
		// Determine the accuracy by calculating the arc length: 
			// arc length = int from a to b { sqrt ((dydt)^2 + (dxdt)^2) } dt 
			acc= ceil (9 * a_y * a_y / 5 + 9 * a_x * a_x / 5 + 3 * a_y * b_y + 3 * a_x * b_x + 2 * c_y * (a_y + b_y) + 2 * c_x * (a_x + b_x) + 
				4 * b_y * b_y / 3 + 4 * b_x * b_x / 3 + c_y * c_y + c_x * c_x); 
			// sprintf (string01, "data: debug:acc=%d\r\n\r\n", acc); 
			// cprint (string01); 
		for (j= 0; j < acc; j++){ 
			...
			// z= sum / bottom; 
			//if (UseDebug) z= 900; 
			if (x < 0 || x >= main_w) continue; 
			if (y < 0 || y >= main_w) continue; 
			px= (DEPTH_PIXEL *)(BitmapData + main_w * 8 * y + 8 * x); 
			if ((int)(px) >= (int)(BitmapData) && (int)(BitmapData) + (main_w * main_h * 8) > (int)(px)){ 
				if (px->depth == PX_UNDEFINED || ((px->depth) > (long)(z * DepthScalar))){ 
					px->r= objOutline[0]; 
...
double AveragePixelDepth (DEPTH_PIXEL ** pixels, int count){ 
	int sum= 0; 
	int i; 
	if (count > 0){ 
		for (i= 0; i < count; i++){ 
	...
	} else return PX_UNDEFINED; 
	return (double)(sum) / (double)(count); 
} 
double ClosestPixelDepth (DEPTH_PIXEL ** pixels, int count, DEPTH_PIXEL * compare){ 
	int closest_i= 0; 
	int closest_d; 
	int tmp; 
	int i; 
	if (count > 0){ 
		closest_d= abs ((pixels[0]->depth) - (compare->depth)); 
		for (i= 1; i < count; i++){ 
			tmp= abs ((pixels[i]->depth) - (compare->depth)); 
			if (tmp < closest_d){ 
				closest_d= tmp; 
				closest_i= i; 
			} 
...
double fdelta (double a, double b){ 
	if (a > b) return a - b; 
	else return b - a; 
} 
		...
		x= i % w; 
			...
			if (mode){ 
				if (mode == 1){ 
					...
					if (i - 1 > 0 && i + 1 < w * h){ 
						if ((int)(&map[i-w-1]) >= (int)(map)){ 
							if ((int)(&map[i+w+1]) + sizeof (DEPTH_PIXEL) < (int)(map) + sizeof (DEPTH_PIXEL) * w * h){ 
								if (px->depth != PX_UNDEFINED){ 
									perimeter[0]= &map[i-1]; 
									perimeter[1]= &map[i+1]; 
									perimeter[2]= &map[i-w]; 
									perimeter[3]= &map[i+w]; 
									perimeter[4]= &map[i-w-1]; 
									perimeter[5]= &map[i+w+1]; 
									perimeter[6]= &map[i+w-1]; 
									perimeter[7]= &map[i-w+1]; 
									// a8avg= AveragePixelDepth (perimeter, 8); 
									// a8cls= ClosestPixelDepth (perimeter, 8, &map[i]); 
									// if (fdelta (a8avg, px->depth) > fdelta (a8cls, px->depth)){ 
										// clr->r= 0; 
										// clr->g= 0; 
										// clr->b= 0; 
									// } 
									for (j= 0; j < 4; j++){ 
										if (perimeter[j*2]->depth == PX_UNDEFINED || perimeter[j*2+1]->depth == PX_UNDEFINED){ 
											clr->r= toon_outline[0]; 
											clr->g= toon_outline[1]; 
											clr->b= toon_outline[2]; 
											j= 4; 
											continue; 
										} 
										a8avg= AveragePixelDepth (&perimeter[j*2], 2); 
										a8cls= ClosestPixelDepth (&perimeter[j*2], 2, px); 
										if (fdelta (a8avg, px->depth) > fdelta (a8cls, px->depth)){ 
											clr->r= toon_outline[0]; 
											clr->g= toon_outline[1]; 
											clr->b= toon_outline[2]; 
						...
						// if (x > 0 && x + 1 < w && ((a1= map[i-1].depth) == PX_UNDEFINED || (a1b= map[i+1].depth) == PX_UNDEFINED || 
							// (((tmp1= (double)(abs (px->depth - a1)) / (double)(abs (px->depth - a1b))) > a2c || tmp1 < 1 / a2c) 
							// && (!(px->depth >= a1) || !(px->depth >= a1b))))){ 
							// clr->r= 0; 
							// clr->g= 0; 
							// clr->b= 0; 
						// } 
					...
					// if (y > 0 && y + 1 < h){ 
						// // if (i - w > 0 && i + w + 1 < w * h){ 
							// // a1= map[i-w].depth; 
							// // a1b= map[i+w].depth; 
							// // if ((a1 == PX_UNDEFINED) || (a1b == PX_UNDEFINED) || 
							// // (((tmp1= (double)(abs (px->depth - a1)) / (double)(abs (px->depth - a1b))) > a2c || tmp1 < 1 / a2c) 
							// // && (!(px->depth >= a1) || !(px->depth >= a1b)))){ 
								// // clr->r= 0; 
								// // clr->g= 0; 
					...
					// if (i - 1 > 0 && i + 1 < w * h){ 
						// if (x > 0 && x + 1 < w && ((a1= map[i-1].depth) != PX_UNDEFINED && (a1b= map[i+1].depth) != PX_UNDEFINED && 
							// (a3= (double)((double)(abs (px->depth - a1)) / (double)(abs (px->depth - a1b)))))){ 
							// if (a3 < 1) a3= 1 / a3; 
							// if (a3 < a2) a6= a3; 
						// } 
						// /*else { 
					...
					if (i - 1 > 0 && i + 1 < w * h){ 
						if (y > 0 && y + 1 < h){ 
							...
							a8avg= AveragePixelDepth (perimeter, 8); 
							a8cls= ClosestPixelDepth (perimeter, 8, &map[i]); 
							a4= pow (a5, a7 * fdelta (a8avg, map[i].depth) / fdelta (a8cls, map[i].depth)); 
							clr->r= (unsigned char)((int)((double)(clr->r) * a4)); 
							clr->g= (unsigned char)((int)((double)(clr->g) * a4)); 
							clr->b= (unsigned char)((int)((double)(clr->b) * a4)); 
						} 
...
char * EnsureQueryHTTP (char * old_command_line){ 
	...
	method[0]= 0; 
	query[0]= 0; 
	if (GetEnvironmentVariable ("REQUEST_METHOD", method, 512)) count1++; 
	if (GetEnvironmentVariable ("QUERY_STRING", query, 16344)) count1++; 
	if (method[0]){ 
		// cprint ("Method: "); 
		// cprint (method); 
		// cprint ("\r\n"); 
		if (!strcmp (method, "POST")){ 
			if (GetEnvironmentVariable ("CONTENT_TYPE", string2, 512)){ 
				if (!strcmp (string2, "application/x-www-form-urlencoded")){ 
					string3[0]= 48; 
					string3[1]= 0; 
					GetEnvironmentVariable ("CONTENT_LENGTH", string3, 512); 
					// cprint ("Debug: "); 
					// cprint (string3); 
					// cprint ("\r\n"); 
					tmp= atoi (string3); 
					if (tmp > sizeof (string3)) tmp= sizeof (string3); 
					ReadFile (GetStdHandle (STD_INPUT_HANDLE), string3, tmp, &bRead, 0); 
					// fgets (string3, tmp, stdin); 
					string3[bRead]= 0; 
					bRead= strlen (string3); 
					if (bRead && query[0]) strcat (query, "&"); 
					strcat (query, string3); 
				} 
			} 
		} 
	} else return old_command_line; 
	i= 0; 
	while (query[i]){ 
