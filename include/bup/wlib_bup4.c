	...
	for (i= 0; buff[i]; i += 2){ 
		...
		for (j= 0; buf[j]; j += 2){ 
			...
			if (kMax == -1 || (kMax < number && number < at)){ 
				kMax= number; 
				if (buf[j+1]) strcpy (output, buf[j+1]); 
				else *output= 0; 
			} 
		} 
		break; 
...
double LookUpNumberExBin (char ** buff, char * path, double at){ 
	...
	double value; 
	double difference; 
	double rAt= (double)(floor (at * 100)) / 100; 
	for (i= 0; buff[i]; i += 2){ 
		if (strcmp (buff[i], path)) continue; 
		if (!(buf= (char **)(buff[i+1]))) continue; 
		kMax= -1; 
		...
		for (j= 0; buf[j]; j += 2){ 
			...
			difference= (double)(floor (number * 100)) / 100 - rAt; 
			if (!difference) return value; 
			if (kMin > number || kMin == -1){ 
				vMin= value; 
			} 
			if (kMax < number || kMax == -1){ 
				vMax= value; 
			} 
		} 
		break; 
		...
		x= i % w; 
			...
			if (mode){ 
				if (mode == 1){ 
					...
					if (x > 0 && ((a1= map[i-1].depth) == PX_UNDEFINED || abs (px->depth - a1) > DepthScalar * a2)){ 
						clr->r= 0; 
						clr->g= 0; 
						clr->b= 0; 
					} 
...
char ** ImportPropertiesBin (char * buffer){ 
	...
	do { 
		...
		buf= (char **)(pBuffer[j+1]= GlobalAlloc (GMEM_ZEROINIT, (c * 2 + 4) * sizeof (char *))); 
		k= 0; 
